...
inputpathway=['/archive/cai/raw_data/xray/test/ljx_step4/back_up']
...
##write the csv file
f1=open(outputpathway+'/'+'crg_detail_aa3_5.csv', 'w')
...
for index,state in enumerate(strucs):
    ...
    for i in range(len(clusts)):
        if (clusts[i] != 'BNC' and clusts[i] != 'P-exit' and clusts[i] != 'unknown' ):
            ...
            plt.ylim(-5.0,2.0)
    plt.savefig(outputpathway+'/'+'Crg_State_'+state+'_aa3_3.png', dpi=300)
    plt.clf()
'''
#print all_state_data_dict
