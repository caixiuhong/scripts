...
def group_crgseq():
	...
	for k, g in groupby(grouped_ms_states, key=lambda item: item.crgseq):
		grouped_crgseqs.append([k, list(g)])
...
def save_excel(file, sheetname):
	...
	for index, i in enumerate(grouped_crgs):
		...
		#print out second, third and forth lowest microstate
		j=1
		while j<= 3:
			if (i[1][j].E -i[1][0].E ) > 4: break
			E_self_total, E_pw, E_mfe, E_self_data, E_pw_matrix, state_res_mfe=analyze_state_energy(i[1][j].confseq, ph=7.0, eh=0.0, cutoff=cutoff)
			all_states.append([index, i[0] ,i[1][j].crgseq,i[1][j].E, E_self_total, E_pw, E_mfe, i[1][j].occ, \
                        i[1][j].confseq,i[1][j].confidseq]) 
			j+=1
...
def write_ms_occ(file, sheetname):
	group_crg()
	...
	detailed_data=defaultdict(dict)
	for index, i in enumerate(grouped_crgs):
		ms_occ=[x for x in i[1] if x.occ > 0.001]
		crg=i[0]
		for j in ms_occ:	
			#E_self_total, E_pw, E_mfe, E_self_data, E_pw_matrix, state_res_mfe=analyze_state_energy(j.confseq, ph=7.0, eh=0.0, cutoff=cutoff)
			#detailed_data[crg]={"E_self": E_self_data, "E_pw": E_pw_matrix, "E_mfe": state_res_mfe}
			#all_states.append([index, i[0] ,j.crgseq,j.E, E_self_total, E_pw, E_mfe, j.occ, \
			#j.confseq,j.confidseq])
			all_states.append([index, i[0] ,j.crgseq,j.E, j.occ, \
			j.confseq,j.confidseq])


	#all_states=pd.DataFrame(all_states,columns=["iState","Tot_crg","Crg_seq","Min_E(Kcal)","E_self(Kcal)",\
	#	"E_pw(Kcal)","E_mfe(Kcal)","Occ","Conf_ids","Conf_Names"])
	all_states=pd.DataFrame(all_states,columns=["iState","Tot_crg","Crg_seq","E(Kcal)",\
		"Occ","Conf_ids","Conf_Names"])
...
def write_ms_crgseqs(file, sheetname):
	group_crgseq()

	all_crgseq_states = []
	for index, i in enumerate(grouped_crgseqs):
		all_crgseq_state=[[x.E, x.occ, x.confseq] for x in i[1]]
		all_crgseq_states.append([index,i[1][0].crg, i[0], all_crgseq_state])
		#all_crgseq_states.append([index, i[1][0].crg, i[0]])


	all_crgseq_states = pd.DataFrame(all_crgseq_states, columns=["iState", "Tot_crg", "Crg_seq", "[E(Kcal), Occ, Confseq]"])
	#all_crgseq_states = pd.DataFrame(all_crgseq_states,
	#								 columns=["iState", "Tot_crg", "Crg_seq"])
	##wrtie data into excel file
	writer = pd.ExcelWriter(file, engine="openpyxl")
	if os.path.isfile(file):
		writer.book = load_workbook(file)

	all_crgseq_states.to_excel(writer, sheet_name=sheetname)
...
def print_group_crg():
        ...
        all_states = []
	all_Es=[]  # store all energys for each ionization state
        for index, i in enumerate(grouped_crgs):
                crg=i[0]
                crg_ms=i[1]
	...
	#draw hist of each ionization group at one figure
	for x in all_Es:
		plt.hist(x[1], bins='auto', label='crg='+str(x[0]), orientation="vertical",alpha=0.7, rwidth=0.85)
		print('number of microstate for %i ionization state: %i' % (x[0], len(x[1])))
	plt.xlabel('State Energy')
        plt.ylabel('Frequency')
        plt.title(struct_state+'Energy histogram at all ionization states')
        plt.grid(axis='y', alpha=0.75)
        plt.legend(loc='best')
        plt.savefig(outputpath+'/'+struct_state+'hist_all_states.png',dpi=300)
        plt.clf()
...
if __name__ == '__main__':
	...
	outputfile='/home/cai/Dropbox/cai/btype_cco/crg_data/quick_run_mdlip/clustering/PLS/ms_crgseq_e1.xlsx'
	#outputfile='./ms_E.xlsx'
	struct_state=sheetname	
	group_ms()
	write_ms_crgseqs(outputfile, sheetname)
